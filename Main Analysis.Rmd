...
pWeightFemale <- ggplot(cdFemale, aes(x = cdFemale$Days, y = cdFemale$Weight, color = cdFemale$Condition)) +
  geom_smooth() +
  labs(x = "Day", y = "Weight (g)", title = "Weight of Female Mice") +
...
pPercWeightFemale <- ggplot(cdFemale, aes(x = cdFemale$Days, y = cdFemale$PercWeight, color = cdFemale$Condition)) +
  geom_smooth() +
  labs(x = "Day", y = "Percent Weight", title = "Proportional Weight of Female Mice") +
  scale_colour_manual("", values = cols, breaks = c("Lab Chow", "Lab Fat", "Wild Chow", "Wild Fat")) +
  theme_set(theme_bw(base_size = 20)) +
  theme(plot.title = element_text(hjust = 0.5))
...
#subset data for female Wildling high fat mice
cdFWF <- subset(cdFemale, cdFemale$Condition == "Wild Fat")

#plot weight by weight gain bin
pBinWeight <- ggplot(cdFWF, aes(x = cdFWF$Days, y = cdFWF$PercWeight, color = cdFWF$WeightGain)) +
  geom_smooth() +
  labs(x = "Day", y = "Proportional Weight (%)", title = "Female Wildling High Fat Mice") +
...
#plot glucose by weight bin
pBinGlucose <- ggplot(cdFWF, aes(x = cdFWF$Days, y = cdFWF$Glucose, color = cdFWF$WeightGain)) +
  geom_smooth() +
...
##Plot temperature
pTemp <- ggplot(cdFemale, aes(x = cdFemale$Days, y = cdFemale$Temperature, color = cdFemale$WeightGain)) +
  geom_smooth() +
  labs(x = "Day", y = "Temperature (°C)", title = "Female Mice") +
  scale_colour_manual("", values = cols, breaks = c("Lab Chow", "Lab Fat", "Wild Chow", "Wild Fat")) +
  theme_set(theme_bw(base_size = 20)) +
  theme(plot.title = element_text(hjust = 0.5))
...
##Find proportional temperature
  #intialize proportional temperature variable
# cd$PercTemp <- NA
# 
# #for each mouse
# for (mouse in unique(cd$UID)) {
#  
#   #find list of measurement days for mouse
#   mouseDays <- sort(unique(cd$Days[cd$UID == mouse]))
#   
#   #find minimum day (start day) and minimum weight (on start day)
#   i = 1
#   minDay <- mouseDays[i]
#   while (is.na(cd$Temperature[cd$UID == mouse & cd$Days == mouseDays[i]])) {
#     i = i + 1
#   }
#   minTemp <- cd$Temperature[cd$UID == mouse & cd$Days == mouseDays[i]]
#  
#   #find proportional weight for the current mouse
#   cd$PercTemp[cd$UID == mouse] <- cd$Temperature[cd$UID == mouse] / minTemp
# }
# 
# #convert proportional weight to a percentage
# cd$PercWeight <- cd$PercWeight * 100
...
##Plot temperature by weight gain bin
pBinTemp <- ggplot(cdFWF, aes(x = cdFWF$Days, y = cdFWF$Temperature, color = cdFWF$WeightGain)) +
  geom_smooth() +
...
```{r glucose, eval = FALSE, include = FALSE}
