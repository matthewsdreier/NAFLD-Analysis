...
##Plot proportional weight
  #define boundaries
minPercWeightFemale <- min(cdFemale$PercWeight)
maxPercWeightFemale <- max(cdFemale$PercWeight)

#create graph
pPercWeightFemale <- ggplot(cdFemale, aes(x = cdFemale$Days, y = cdFemale$PercWeight, color = cdFemale$Color)) +
  geom_smooth() +
...
barplot(prop.table(table(cd$WeightGain[cd$Days == 0], cd$Condition[cd$Days == 0]), margin = 2), col = c("black", "light gray"))

legend("bottomright", inset = 0.02, legend = c("High", "Low"), fill = c("black", "light gray"))
