...
for (Tag in unique(LFat$Days)  ) {
  varLFat <- (LFat$Weight[LFat$Days == Tag])
  MeanLabFat <- mean(varLFat , na.rm = TRUE)
print(MeanLabFat)

}

for (Tag in unique(WildFat$Days)  ) {
  varWildFat <- (WildFat$Weight[WildFat$Days == Tag])
  MeanWildFat <- mean(varWildFat , na.rm = TRUE)
print(MeanWildFat)
...
t.test(MeanLabFat~MeanWildFat, mu=0, alt="two.sided", conf=0.95, var.eq=F, paired=F)
